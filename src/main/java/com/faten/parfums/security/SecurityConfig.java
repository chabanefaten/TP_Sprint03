package com.faten.parfums.security;
...
@Configuration
	...
	.requestMatchers("/showCreate","/saveParfum")
	.hasAnyAuthority("ADMIN","AGENT")
	
	.requestMatchers("/modifierParfum","/supprimerParfum")
	.hasAuthority("ADMIN")
	 
	.requestMatchers("/ListeParfums").hasAnyAuthority("ADMIN","AGENT","USER")
	...
	@Bean
	 ...
	 return new BCryptPasswordEncoder();
		 ...
		 * .authorities("ADMIN") .build(); UserDetails userFaten = User
		 * .withUsername("faten") .password(passwordEncoder.encode("123"))
		 * .authorities("AGENT", "USER") .build(); UserDetails user1 = User
		 * .withUsername("user1") .password(passwordEncoder.encode("123"))
		 * .authorities("USER") .build();
		 * 
		 * return new InMemoryUserDetailsManager(admin, userFaten, user1); }
		 */
	}
